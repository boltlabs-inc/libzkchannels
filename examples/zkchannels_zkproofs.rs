...
fn main() {
    ...
    let (com, com_proof, est_time) = measure_two_arg!(zkproofs::establish_customer_generate_proof(rng, &mut channel_token, &mut cust_state));
    println!(">> Time to generate proof for establish: {} ms", est_time);

    // obtain close token for closing out channel
    let channel_id = channel_token.compute_channel_id();
    let option = zkproofs::establish_merchant_issue_close_token(rng, &channel_state, &com, &com_proof,
    ...
    let (payment, new_cust_state, pay_time) = measure_two_arg!(zkproofs::generate_payment_proof(rng, &channel_state, &cust_state, pay_inc));
    println!(">> Time to generate payment proof: {} ms", pay_time);

    let (new_close_token, verify_time) = measure_one_arg!(zkproofs::verify_payment_proof(rng, &channel_state, &payment, &mut merch_state));
    ...
    let (payment2, new_cust_state2, pay_time2) = measure_two_arg!(zkproofs::generate_payment_proof(rng, &channel_state, &cust_state, pay_inc2));
    println!(">> Time to generate payment proof 2: {} ms", pay_time2);

    let (new_close_token2, verify_time2) = measure_one_arg!(zkproofs::verify_payment_proof(rng, &channel_state, &payment2, &mut merch_state));
    println!(">> Time to verify payment proof 2: {} ms", verify_time2);

    let revoke_token2 = zkproofs::generate_revoke_token(&channel_state, &mut cust_state, new_cust_state2, &new_close_token2);
