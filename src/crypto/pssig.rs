...
impl<E: Engine> BlindPublicKey<E> {
    ...
    pub fn verify_proof(
        ...
        let mut gx_rhs = E::G2::zero();
        gx_rhs.add_assign(&self.X2);
        gx_rhs.mul_assign(E::Fr::from(challenge));
        for j in 0..self.Y2.len() {
            let mut new_term = E::G2::zero();
            new_term.add_assign(&self.Y2[j]);
            new_term.mul_assign(E::Fr::from(p.zsig[j]));
            gx_rhs.add_assign(&new_term);
        }
        let mut new_term = E::G2::zero();
        new_term.add_assign(&mpk.g2);
        new_term.mul_assign(p.zv.into_repr());
        gx_rhs.add_assign(&new_term);
        let gx = E::pairing(blindSig.h, gx_rhs);

        let mut g_rhs = E::G2::zero();
        g_rhs.add_assign(&mpk.g2);
        g_rhs.mul_assign(E::Fr::from(challenge));
        let mut g = E::pairing(blindSig.H, g_rhs);
        g.mul_assign(&p.a);
        gx == g
    }
    ...
    fn prove_commitment_inefficient<R: Rng>(
        &self,
        rng: &mut R,
        mpk: &PublicParams<E>,
        signature: &Signature<E>,
    ...
    pub fn prove_commitment<R: Rng>(
        ...
        for j in 0..self.Y2.len() {
            let mut new_term = E::G2::zero();
            new_term.add_assign(&self.Y2[j]);
            new_term.mul_assign(t[j].into_repr());
            rhs.add_assign(&new_term);
        }
...
mod tests {
    ...
    fn compare_signature_commitment_algorithms() {
        ...
        let s = Instant::now();
        let ps_old = keypair.public.prove_commitment_inefficient(&mut rng_old, &mpk, &sig, Some(ts.clone()), Some(tt.clone()));
        let e_old = s.elapsed();
    ...
    #[test]
    fn compare_signature_proof_verification_algorithms() {
        let mut rng = &mut rand::thread_rng();
        ...
        let s = Instant::now();
        let pass_old = keypair
            .public
            .verify_proof_inefficient(&mpk, &proof_state.blindSig, &proof, challenge);
        let e_old = s.elapsed();
        let pass_new = keypair
            .public
            .verify_proof(&mpk, &proof_state.blindSig, &proof, challenge);
        let e_new = s.elapsed();
