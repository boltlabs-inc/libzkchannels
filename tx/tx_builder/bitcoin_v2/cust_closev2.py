...
if args.debug:
    ...
    print("\nParts within outputs: ")
    print("cust_script_output_value: ", cust_script_output_value.hex())
    print("len(cust script): ", (len(cust_scriptPK)).to_bytes(1, byteorder="little", signed=False).hex())
    print("cust_scriptPK:, ", cust_scriptPK.hex())

    print("merch_payout_value little endian: ", merch_payout_value.hex())
    print("len(merch payout scriptPK): ", (len(merch_payout_scriptPK)).to_bytes(1, byteorder="little", signed=False).hex())
    print("merch_payout_scriptPK: ",merch_payout_scriptPK.hex())
    print("(merch_payout_pubkey hash: ",hash160(merch_payout_pubkey).hex(),")")

    print("op_return_output_value little endian: ", op_return_output_value.hex())
    print("len(op_return output): ", (len(op_return_scriptPK)).to_bytes(1, byteorder="little", signed=False).hex())
    print("op_return_scriptPK: ",op_return_scriptPK.hex())

    print("\nCust close script: ")
    print(cust_close_script.hex())

    print("\nParts within cust script: ")
    print("revocation_lock: ",revocation_lock.hex())
    print("merch_dispute_pubkey: ", merch_dispute_pubkey.hex())
    print("len(delay) + to_self_delay: ", len(to_self_delay_little_e).to_bytes(1, byteorder="little", signed=False).hex()
        + to_self_delay_little_e.hex())
    print("cust_payout_pubkey: ",cust_payout_pubkey.hex())
