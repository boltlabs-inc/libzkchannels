...
parser.add_argument("--merch_disp_privkey", "-mdsk", help="private key of merch dispute output")
parser.add_argument("--revocation_secret", "-rs", help="revocation secret")
parser.add_argument("--cust_close_pubkey", "-ccpk", help="public key of cust close to-self output")
parser.add_argument("--output_pubkey", "-cpk", help="pubkey of output for the merch")
...
merch_disp_privkey = bytes.fromhex(args.merch_disp_privkey)
merch_disp_pubkey = privkey_to_pubkey(merch_disp_privkey)

revocation_secret = bytes.fromhex(args.revocation_secret)
revocation_lock = hash160(revocation_secret)
...
signing_key_merch_disp = ecdsa.SigningKey.from_string(merch_disp_privkey, curve=ecdsa.SECP256k1) # Don't forget to specify the curve
signature_merch_disp = signing_key_merch_disp.sign_digest(hashed_bip_143, sigencode=ecdsa.util.sigencode_der_canonize)
...
witness = (
    ...
    # signature
    + (len(signature_merch_disp)+1).to_bytes(1, byteorder="little", signed=False)
    + signature_merch_disp
    + sighash_type_flag

    + (len(revocation_secret)).to_bytes(1, byteorder="little", signed=False)
    + revocation_secret
