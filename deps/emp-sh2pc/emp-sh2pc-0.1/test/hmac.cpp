...
string run_secure_HMACsign(string key, string msg) {
  ...
  Constants constants = distribute_Constants(MERCH);
  Integer k[64];
  Integer H[8];
  initSHA256(k, H);

  HMACsign(merch_key_d, state_d, paytoken_d.paytoken, constants, k, H);

  Integer hash = composeSHA256result(paytoken_d.paytoken, constants.thirtytwo);
