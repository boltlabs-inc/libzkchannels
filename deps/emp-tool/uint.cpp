...
void test_int(int party, int range1 = 1<<25, int range2 = 1<<25, int runs = 1000) {
    ...
    for(int i = 0; i < runs; ++i) {
        ...
        UInteger a(32, ia, ALICE); 
        UInteger b(32, ib, BOB);

        UInteger res = Op2()(a,b);

        if (res.reveal<uint>(PUBLIC) != Op()(ia,ib)) {
            cout << ia <<"\t"<<ib<<"\t"<<Op()(ia,ib)<<"\t"<<res.reveal<int>(PUBLIC)<<endl<<flush;
        }
        assert(res.reveal<uint>(PUBLIC) == Op()(ia,ib));
    }
    cout << typeid(Op2).name()<<"\t\t\tDONE"<<endl;
}

void scratch_pad() {
    UInteger a(32, 9, ALICE);
...
//	scratch_pad();return 0;
    test_int<std::plus<uint>, std::plus<UInteger>>(party);
    /*
    ...
    */
    test_int<std::bit_and<uint>, std::bit_and<UInteger>>(party);
    test_int<std::bit_or<uint>, std::bit_or<UInteger>>(party);
    test_int<std::bit_xor<uint>, std::bit_xor<UInteger>>(party);
